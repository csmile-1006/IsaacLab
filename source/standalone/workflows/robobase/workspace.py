...
def relabel_with_predictor(reward_model, replay_buffer):
    ...
    for ep_fn in episodes:
        ...
        new_episode = reward_model.compute_reward(episode, _obs_signature=_obs_signature)
...
def _create_default_replay_buffer(
    ...
    return replay_class(
        save_dir=cfg.replay.save_dir if not demo_replay else cfg.replay.save_dir + "_demo",
...
def _create_default_query_replay_buffer(
    ...
    if cfg.demos > 0:
        batch_size = (
            (cfg.reward_replay.num_queries // 2 + 1 if not use_demo else cfg.reward_replay.num_queries // 2)
...
class Workspace:
    def __init__(
        ...
        self.work_dir = Path(
            hydra.core.hydra_config.HydraConfig.get().runtime.output_dir if work_dir is None else work_dir
        )
        ...
        if self.use_rlhf:
            ...
            if cfg.rlhf.feedback_type == "gemini":
                cfg.rlhf.gemini.task_description = self.env_factory.get_task_description(cfg)
        ...
        if self.use_demo_replay:
            self.demo_replay_buffer = create_replay_fn(cfg, observation_space, action_space, demo_replay=True)
        ...
        if self.prioritized_replay:
            if self.use_demo_replay:
                raise NotImplementedError("Demo replay is not compatible with prioritized replay")
    ...
    def global_env_steps(self):
        ...
        return (
            self._main_loop_iterations * self.cfg.action_repeat * self.train_envs.num_envs * self.cfg.action_sequence
            + self.pretrain_steps
        )
    ...
    def replay_iter(self):
        if self._replay_iter is None:
            ...
            if self.use_demo_replay:
                ...
                _replay_iter = utils.merge_replay_demo_iter(_replay_iter, _demo_replay_iter)
    ...
    def query_replay_iter(self):
        ...
        if self._query_replay_iter is None:
            ...
            if self.use_demo_replay:
                ...
                _query_replay_iter = utils.merge_replay_demo_iter(_query_replay_iter, _demo_query_replay_iter)
    ...
    def _add_to_replay(
        ...
        list_of_obs_dicts = [dict(zip(observations, t)) for t in zip(*observations.values())]
        agents_reset = []
        for i in range(self.train_envs.num_envs):
            ...
            if terminations[i] or truncations[i]:
                ...
                relabeling_as_demo = task_success and self.use_demo_replay and self.cfg.use_self_imitation
                ep_index = 0
                for act, obs, rew, term, trunc, info, next_info in ep:
                    ...
                    extra_replay_elements = {k: v for k, v in info.items() if k in self.extra_replay_elements.keys()}

                    self.replay_buffer.add(obs, act, rew, term, trunc, **extra_replay_elements)
                    if relabeling_as_demo:
                        self.demo_replay_buffer.add(obs, act, rew, term, trunc, **extra_replay_elements)
                    if self.use_rlhf:
    ...
    def _load_demos(self):
        if (num_demos := self.cfg.demos) > 0:
            ...
            if self.use_demo_replay:
                ...
                self.env_factory.load_demos_into_replay(self.cfg, self.demo_replay_buffer)
            if self.use_rlhf:
                # Load demos to the dedicated query_replay_buffer
                self.env_factory.load_demos_into_replay(self.cfg, self.query_replay_buffer)
                if self.use_demo_replay:
                    self.env_factory.load_demos_into_replay(self.cfg, self.demo_query_replay_buffer)
    ...
    def _perform_updates(self) -> dict[str, Any]:
        ...
        if self.agent.logging:
            ...
            metrics["agent_batched_updates_per_second"] = self.train_envs.num_envs / execution_time_for_update
    ...
    def collect_feedback(self):
        ...
        for feedback in feedbacks:
            self.feedback_replay_buffer.add_feedback(feedback["segment_0"], feedback["segment_1"], feedback["label"])
    ...
    def _perform_reward_model_updates(self) -> dict[str, Any]:
        ...
        if self.reward_model.logging:
            execution_time_for_update = time.time() - start_time
            metrics["reward_model_batched_updates_per_second"] = 1 / execution_time_for_update
            metrics["agent_updates_per_second"] = (
    ...
    def _perform_env_steps(
        ...
        with torch.no_grad(), utils.eval_mode(self.agent):
            torch_observations = {k: torch.from_numpy(v).to(self.device) for k, v in observations.items()}
            if eval_mode:
                torch_observations = {k: v.unsqueeze(0) for k, v in torch_observations.items()}
            action = self.agent.act(torch_observations, self.main_loop_iterations, eval_mode=eval_mode)
            metrics = {}
            ...
            if action.ndim != 3:
                raise ValueError("Expected actions from `agent.act` to have shape " "(Batch, Timesteps, Action Dim).")
            if eval_mode:
                action = action[0]  # we expect batch of 1 for eval

        if self.agent.logging:
            execution_time_for_act = time.time() - start_time
            metrics["agent_act_steps_per_second"] = self.train_envs.num_envs / execution_time_for_act
            start_time = time.time()
        ...
        *env_step_tuple, next_info = env.step(action)
        ...
        if self.agent.logging:
            ...
            metrics["env_steps_per_second"] = self.train_envs.num_envs / execution_time_for_env_step
        ...
        if self.cfg.num_pretrain_steps > 0:
            ...
            while pre_train_until_step(self.pretrain_steps):
                ...
                if should_pretrain_log(self.pretrain_steps):
                    ...
                    self.logger.log_metrics(pretrain_metrics, self.pretrain_steps, prefix="pretrain")

                if should_pretrain_eval(self.pretrain_steps):
                    eval_metrics = self._eval()
                    eval_metrics.update(self._get_common_metrics())
                    self.logger.log_metrics(eval_metrics, self.pretrain_steps, prefix="pretrain_eval")
    ...
    def _pretrain_reward_model_on_demos(self):
        if self.cfg.rlhf.num_pretrain_steps > 0:
            ...
            should_pretrain_log = utils.Every(self.cfg.log_pretrain_every)
            if self.cfg.log_pretrain_every > 0:
                assert self.cfg.rlhf.num_pretrain_steps % self.cfg.log_pretrain_every == 0
    ...
    def _online_rl(self):
        ...
        snapshot_reward_model_every_n = self.cfg.rlhf.snapshot_every_n if self.cfg.save_snapshot else 0
        ...
        while train_until_frame(self.global_env_steps):
            ...
            if self.use_rlhf:
                ...
                if (
                    ...
                    and (self.main_loop_iterations > 0 or self.reward_pretrain_steps == 0)
                ):
                    self.reward_model.logging = True
                    logging.info(
                    ...
                    for it in range(self.cfg.rlhf.num_train_frames):
                        ...
                        if should_log(it):
                            self.logger.log_metrics(metrics, self.global_env_steps, prefix="train_reward")
                    relabel_with_predictor(self.reward_model, self.replay_buffer)
                    if self.use_demo_replay:
                        relabel_with_predictor(self.reward_model, self.demo_replay_buffer)
                    metrics = {}

                if self.total_feedback < self.cfg.rlhf.max_feedback and should_save_reward_model_snapshot(
                    self.main_loop_iterations
                ):
                    self.save_reward_model_snapshot()
            ...
            if should_eval(self.main_loop_iterations):
                eval_metrics = self._eval()
                eval_metrics.update(self._get_common_metrics())
                self.logger.log_metrics(eval_metrics, self.global_env_steps, prefix="eval")
    ...
    def load_snapshot(self, path_to_snapshot_to_load=None):
        if path_to_snapshot_to_load is None:
            path_to_snapshot_to_load = self.work_dir / "snapshots" / "latest_snapshot.pt"
        else:
            path_to_snapshot_to_load = Path(path_to_snapshot_to_load)
        if not path_to_snapshot_to_load.is_file():
            raise ValueError(f"Provided file '{str(path_to_snapshot_to_load)}' is not a snapshot.")
    ...
    def load_reward_model_snapshot(self, path_to_snapshot_to_load=None):
        if path_to_snapshot_to_load is None:
            path_to_snapshot_to_load = self.work_dir / "reward_model_snapshots" / "latest_snapshot.pt"
        else:
            path_to_snapshot_to_load = Path(path_to_snapshot_to_load)
        if not path_to_snapshot_to_load.is_file():
            raise ValueError(f"Provided file '{str(path_to_snapshot_to_load)}' is not a snapshot.")
